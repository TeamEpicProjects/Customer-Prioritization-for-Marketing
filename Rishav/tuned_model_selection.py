...
fit_algorithm("SDC feature_set_1 std scaled tuned", sgd_model_tuned, X_scaled_s_f1, y)
fit_algorithm("SDC feature_set_3 std scaled tuned", sgd_model_tuned, X_scaled_s_f3, y)
df_final_tuned = fit_algorithm("SDC feature_set_4 std scaled tuned", sgd_model_tuned, X_scaled_s_f4, y)
...
# 2021-09-23 08:55:47     Reading dataset: base_data_resampled_tomek.csv ...

# Splitting into features and lables ...

# 2021-09-23 08:55:47     Reading dataset: Logistic_Tuned_Hyperparameters.csv ...

# 2021-09-23 08:55:47     Reading dataset: SGD_Tuned_Hyperparameters.csv ...
# The best parameters for LOGISTIC REGRESSION models are: -
#                          model                                             best hyperparameters
# 0            LR1 feature_set_1  {'C': 0.001, 'class_weight': 'balanced', 'solver': 'newton-cg'}
# 1            LR1 feature_set_4  {'C': 0.001, 'class_weight': 'balanced', 'solver': 'newton-cg'}
...
# 0             LR1 feature_set_1 tuned              7                   0.980        0.981                    0.979         0.981     4.159       0.037
# 6  SDC feature_set_1 std scaled tuned              7                   0.979        0.980                    0.979         0.980     1.710       0.037
# 1             LR1 feature_set_4 tuned              6                   0.978        0.982                    0.978         0.982     3.136       0.035
# 7  SDC feature_set_3 std scaled tuned              8                   0.976        0.978                    0.976         0.978     1.351       0.038
# 8  SDC feature_set_4 std scaled tuned              6                   0.967        0.955                    0.968         0.957     1.001       0.037
# 4      LR1 feature_set_1 scaled tuned              7                   0.959        0.935                    0.959         0.935   127.024       0.037
# 5      LR1 feature_set_4 scaled tuned              6                   0.959        0.936                    0.959         0.936    92.228       0.037
# 2      LR1 feature_set_1 scaled tuned              7                   0.943        0.901                    0.943         0.901     0.955       0.036
# 3      LR1 feature_set_4 scaled tuned              6                   0.943        0.902                    0.943         0.902     0.787       0.036
