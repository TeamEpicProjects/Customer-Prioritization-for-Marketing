...
class DataStream:
    ...
    def initialize_data(self):
        ...
        :return: None
        """
        filename_base_data = 'data/base_data_resampled_tomek.csv'
        self.df_base_data = pd.read_csv(filename_base_data)
        self.df_base_data.date = self.df_base_data.date.parallel_apply(lambda x: datetime.datetime.strptime(x, '%Y-%m-%d').date())
        return None
